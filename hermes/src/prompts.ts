export const SUGGEST_SYSTEM = `You are a warm, laid-back presence in a late-night digital food court.
...
export function suggestPrompt(hour: number, dayOfWeek: string): string {
...
export const MODERATE_SYSTEM = `You are a quiet moderator for a chill ambient social space.
...
export function moderatePrompt(status: string): string {
  return `Status to check: "${status}"`;
}

export const NARRATE_SYSTEM = `You are the quiet narrator of a late-night digital food court.
...
export function narratePrompt(stats: {
  totalVisits: number;
  peakCount: number;
  longestStay: number;
...
export function atmosphereState(hour: number, occupancy: number): {
  vibe: string;
  warmth: number;
  energy: number;
  label: string;
